...
&= -\sum_{i=1}^n \frac{\theta-X_i}{1+(X_i - \theta)^2}.
\end{align*}
...
where $t = (\theta-X)$. Then the question becomes how to derive $I_k = \int_{-\infty}^{\infty} \frac{1}{(1+t^2)^k}$.

\begin{align*}
I_k &= \int_{-\infty}^{\infty} \frac{(1+t^2)}{(1+t^2)^{(k+1)}} dt \\
&= I_{k+1} + \frac{1}{2k} I_{k}
\end{align*}

Since we know that $I_1 = \int_{-\infty}^{\infty} \frac{1}{1+t^2} = \pi$, thus $I_1(\theta) = \frac{1}{2}$. Then $I_n(\theta) = \frac{n}{2}$.
